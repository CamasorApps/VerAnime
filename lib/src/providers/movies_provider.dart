...
class MoviesProvider {
  ...
  Future<List<Movie>> _process(String endpoint) async {
    try {
      final resp = await http.get(Uri.parse(_url + endpoint));
  ...
  Future<bool> index() async {
    try {
      if (_currentPage <= _pages) {
        _loading = !_loading;
        final resp =
            await http.get(Uri.parse(_url + 'movies?page=$_currentPage'));
  ...
  Future<Movie> show(String id) async {
    try {
      final resp = await http.get(Uri.parse(_url + 'movies/show/' + id));
  ...
  Future<List<MovieVideo>> videos(String id) async {
    try {
      final resp = await http.get(Uri.parse(_url + 'movies/videos/' + id));
  ...
  void view(id) {
    http.get(Uri.parse(_url + 'movies/view/' + id));
  }
}
